...
Describe "Testing with files" {
    It "array by pipeline" {
        ...
        $file3 = New-Item -Path 'Testdrive:\File3' -ItemType File
        $file2.CreationTime = $file1.CreationTime.AddDays(1)
        $file3.CreationTime = $file1.CreationTime.AddDays(2)
        ...
        # Assert results
        $ret[0].CreationTime | Should -Be $file1.CreationTime
        $ret[1].CreationTime | Should -Be $file2.CreationTime
        $ret[2].CreationTime | Should -Be $file3.CreationTime
        $ret[0].AboveAvg | Should -Be $false
        $ret[1].AboveAvg | Should -Be $false
        $ret[2].AboveAvg | Should -Be $true
    }
}
